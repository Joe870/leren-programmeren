...
def getEarnigs(profitGold:float, mainCharacter:dict, friends:list, investors:list) -> list:
    ...
    earnings = []
    # alle aventuurlijke vrienden
    adventuringFriends = getAdventuringFriends(friends)
    # interessante investors 
    interestingInvestors = getInterestingInvestors(investors)
    # interessante investors die mee op avontuur
    adventuringInvestors = getAdventuringInvestors(investors)
    # berekent het geld wat iedere investeerder krijgt
    investorsCuts = getInvestorsCuts(profitGold, investors)
    # het goud dat overblijft na de investors
    goldCut = profitGold - sum(investorsCuts)
    endGold = 0
    for person in people: 
        # haalt de naam uit 
        name = person['name'] 
        # berekent hoeveel goud hij nu bij zich heeft
        startGold = getCashInGoldFromPeople([person]) 
        # hoeveel goud een investeerder krijgt die mee is gegaan op avontuur
